...
import { createAirline } from "@/actions/airline"
...
export default function NewAirlinePage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl space-y-6">
      <div>
        <h1 className="font-heading text-xl font-bold text-[#0B3C6D]">New Airline</h1>
        <p className="text-sm text-[#9CA3AF]">Add a new airline</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-1 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-blue-400" placeholder="e.g. Garuda Indonesia" />
          </div>
          <div>
            <label className="text-xs font-medium text-gray-500">IATA Code</label>
            <input type="text" value={form.iataCode} onChange={(e) => setForm({ ...form, iataCode: e.target.value.toUpperCase() })} className="mt-1 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-blue-400" placeholder="GA" maxLength={2} />
          </div>
          <div>
            <label className="text-xs font-medium text-gray-500">ICAO Code</label>
            <input type="text" value={form.icaoCode} onChange={(e) => setForm({ ...form, icaoCode: e.target.value.toUpperCase() })} className="mt-1 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-blue-400" placeholder="GIA" maxLength={3} />
          </div>
          <div>
          ...
          <div>
            ...
            <input type="text" value={form.website} onChange={(e) => setForm({ ...form, website: e.target.value })} className="mt-1 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-blue-400" placeholder="https://..." />
          </div>
          <div>
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <button type="submit" disabled={saving} className="flex items-center gap-1.5 rounded-xl bg-[#0B3C6D] px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-[#0B3C6D]/20 hover:bg-[#0B2D52] disabled:opacity-50">
            {saving ? "Saving..." : <><Save className="size-4" /> Save Airline</>}
          </button>
        </div>
      </form>
