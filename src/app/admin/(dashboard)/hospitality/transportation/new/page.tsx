...
import { createTransportation } from "@/actions/transportation"
import { MediaPicker } from "@/components/admin/media/media-picker"

const TYPE_OPTIONS = ["BUS", "HIACE", "COASTER", "SUV", "SEDAN", "TRAIN", "SHIP"]

export default function NewTransportationPage() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl space-y-6">
      <div>
        <h1 className="font-heading text-xl font-bold text-[#0B3C6D]">New Transportation</h1>
        <p className="text-sm text-[#9CA3AF]">Add a new transportation</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div className="md:col-span-2">
            ...
            <input type="text" value={form.name} onChange={(e) => setForm({ ...form, name: e.target.value })} className="mt-1 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-blue-400" placeholder="e.g. Bus 50 Seat" />
          </div>
          <div>
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <button type="submit" disabled={saving} className="flex items-center gap-1.5 rounded-xl bg-[#0B3C6D] px-4 py-2.5 text-sm font-semibold text-white shadow-lg shadow-[#0B3C6D]/20 hover:bg-[#0B2D52] disabled:opacity-50">
            {saving ? "Saving..." : <><Save className="size-4" /> Save</>}
          </button>
        </div>
      </form>
