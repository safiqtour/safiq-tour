...
import { getTag, updateTag } from "@/modules/business/tag/actions/tag"
import type { TagListItem } from "@/modules/business/tag/types"

const ICON_OPTIONS = [
  "Tag", "Star", "Award", "Sparkles", "Plane", "Users", "Gift",
  "BadgePercent", "Clock", "Heart", "Flame", "Zap", "Globe", "Moon", "Sun",
]
...
export default function EditTagPage() {
  ...
  const [form, setForm] = useState({
    ...
    color: "blue", icon: "Tag", displayOrder: "0", status: "ACTIVE",
  })

  useEffect(() => {
    getTag(params.id as string).then((res) => {
      const f = res as unknown as TagListItem | null
      if (f) {
        setForm({
          name: f.name ?? "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updateTag(params.id as string, { ...form, displayOrder: Number(form.displayOrder) })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl space-y-6">
      <div>
        <h1 className="font-heading text-xl font-bold text-[#0B3C6D]">Edit Tag</h1>
        <p className="text-sm text-[#9CA3AF]">Update tag information</p>
      </div>
      {error && <div className="rounded-xl bg-red-50 border border-red-200 px-4 py-3 text-sm text-red-700">{error}</div>}
      <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          <div>
            <label className="text-xs font-medium text-gray-500">Tag Name *</label>
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <button type="submit" disabled={saving}
            ...
            {saving ? "Saving..." : <><Save className="size-4" /> Update Tag</>}
          </button>
        </div>
      </form>
    </motion.div>
