...
import { createTag } from "@/modules/business/tag/actions/tag"
...
export default function NewTagPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createTag({ ...form, displayOrder: Number(form.displayOrder) })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl space-y-6">
      <div>
        <h1 className="font-heading text-xl font-bold text-[#0B3C6D]">New Tag</h1>
        <p className="text-sm text-[#9CA3AF]">Add a new universal tag</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
          ...
          <div>
            ...
            <input type="text" value={form.shortName} onChange={(e) => setForm({ ...form, shortName: e.target.value.toUpperCase() })}
              className="mt-1 w-full rounded-xl border border-gray-200 px-3 py-2.5 text-sm outline-none focus:border-blue-400"
              placeholder="e.g. BEST" maxLength={20} />
          </div>
        </div>
        <div>
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <button type="submit" disabled={saving}
            ...
            {saving ? "Saving..." : <><Save className="size-4" /> Save Tag</>}
          </button>
        </div>
      </form>
    </motion.div>
