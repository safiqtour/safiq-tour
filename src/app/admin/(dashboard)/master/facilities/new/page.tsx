...
import { createFacility } from "@/modules/business/facility/actions/facility"
...
export default function NewFacilityPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createFacility({ ...form, sortOrder: Number(form.sortOrder) })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl space-y-6">
      <div>
        <h1 className="font-heading text-xl font-bold text-[#0B3C6D]">New Facility</h1>
        <p className="text-sm text-[#9CA3AF]">Add a new facility</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          >
            {saving ? "Saving..." : <><Save className="size-4" /> Save Facility</>}
          </button>
        </div>
      </form>
    </motion.div>
