...
export default function EditPackageCategoryPage() {
  ...
  useEffect(() => {
    getPackageCategory(params.id as string).then((res) => {
      const f = res as unknown as PackageCategoryListItem | null
      if (f) {
        setForm({
          name: f.name ?? "",
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await updatePackageCategory(params.id as string, { ...form, displayOrder: Number(form.displayOrder) })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl space-y-6">
      <div>
        <h1 className="font-heading text-xl font-bold text-[#0B3C6D]">Edit Package Category</h1>
        <p className="text-sm text-[#9CA3AF]">Update package category information</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          >
            {saving ? "Saving..." : <><Save className="size-4" /> Update Category</>}
          </button>
        </div>
      </form>
    </motion.div>
