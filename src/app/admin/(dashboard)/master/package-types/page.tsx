...
import { Plus, Pencil, Plane, Star } from "lucide-react"
...
} from "@/modules/business/package-type/actions/package-type"
...
export default function PackageTypesPage() {
  ...
  const fetchData = useCallback(async () => {
    ...
    try {
      ...
      setData((result.data ?? []) as unknown as PackageTypeListItem[])
  ...
  useEffect(() => { canUser("master.package-type:create").then(setCanCreate) }, [])
  ...
  const columns: Column<PackageTypeListItem>[] = [
    ...
    { key: "defaultDurationDays", header: "Duration", sortable: true, render: (item) => (
      <span>{item.defaultDurationDays > 0 ? `${item.defaultDurationDays} days` : "—"}</span>
    )},
    { key: "defaultCategory", header: "Category", sortable: false, render: (item) => (
      <span className="text-xs text-[#6B7280]">{item.defaultCategory?.name ?? "—"}</span>
    )},
    { key: "defaultVisa", header: "Visa", sortable: false, render: (item) => (
      <span className="text-xs text-[#6B7280]">{item.defaultVisa?.name ?? "—"}</span>
    )},
    { key: "displayOrder", header: "Order", sortable: true, hideOnMobile: true },
    { key: "isFeatured", header: "Featured", sortable: true, render: (item) => (
      <button
        onClick={() => toggleFeaturedPackageType(item.id).then(fetchData)}
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="space-y-6">
      <div className="flex items-center justify-between">
        ...
        {canCreate && (
          <Link
            href="/admin/master/package-types/new"
      ...
      <DataTable
        ...
        onDelete={(id) => deletePackageType(id).then(fetchData)}
        onRestore={(id) => restorePackageType(id).then(fetchData)}
        getId={(item) => item.id}
        getIsDeleted={(item) => !!item.deletedAt}
        actions={(item) => canCreate ? (
          <Link
            href={`/admin/master/package-types/${item.id}/edit`}
