...
import { createBusinessSetting } from "@/modules/business/business-setting/actions/business-setting"
import { BUSINESS_SETTING_GROUPS, BUSINESS_SETTING_VALUE_TYPES } from "@/modules/business/business-setting/types"

export default function NewBusinessSettingPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      await createBusinessSetting({ ...form, sortOrder: Number(form.sortOrder) })
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} className="max-w-2xl space-y-6">
      <div>
        <h1 className="font-heading text-xl font-bold text-[#0B3C6D]">New Business Setting</h1>
        <p className="text-sm text-[#9CA3AF]">Add a new system configuration</p>
      </div>
      ...
      <form onSubmit={handleSubmit} className="rounded-xl border border-gray-200 bg-white p-6 space-y-4">
        ...
        <div className="flex gap-2 justify-end pt-2">
          ...
          <button type="submit" disabled={saving}
            ...
            {saving ? "Saving..." : <><Save className="size-4" /> Save Setting</>}
          </button>
        </div>
      </form>
    </motion.div>
